...
const RightSidebar = () => {
  ...
  useEffect(() => {
    ...
    setMsgImages(images.length > 0 ? images : [])
  }, [messages])
  ...
  return selectedUser && (
    <div className={`bg-[#818582]/10 text-white w-full relative overflow-y-scroll ${selectedUser ? 'max-md:hidden' : ''}`}>
      ...
      <div className='px-5 text-xs mt-3'>
        <p>Media</p>
      ...
      <button onClick={()=>logout()} className='absolute bottom-5 left-1/2 tranform -translate-x-1/2 bg-gradient-to-r from-purple-400 to-violet-600 text-white border-none text-sm font-light py-2 px-20 rounded-full cursor-pointer '>
        Logout
      </button>
    </div>
  )
